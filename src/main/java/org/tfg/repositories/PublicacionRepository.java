...
public interface PublicacionRepository extends JpaRepository<Publicacion, Long> {
	...
	@Query(value = "SELECT * FROM publicacion WHERE publicacion.duenio_publicacion_id IN (SELECT seguimiento.seguido_id FROM seguimiento WHERE seguimiento.seguidor_id = :id AND seguimiento.aceptado = 1)\n"
			+ "", nativeQuery = true)
